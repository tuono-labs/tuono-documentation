...
Create the folder `routes/pokemons` and add two new files: `[pokemon].tsx` and `[pokemon].rs`.

The new route will handle every request that points to e.g. `http://localhost:3000/pokemons/bulbasaur`.
...
For the interface, paste the following snippet in `[pokemon].tsx`:
...
And let's add a stylesheet to give the finishing touch:
