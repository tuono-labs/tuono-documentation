...
/**
 * Hook inspired by: https://github.com/mantinedev/mantine/tree/master/apps/mantine.dev/src/components/TableOfContents
 */
...
export function usePageHeadings(): Array<Heading> {
  const [headings, setHeadings] = useState([])
  const { pathname } = useRouter()

  useEffect(() => {
    const root = document.getElementById('mdx-root')
    setHeadings(Array.from(root?.querySelectorAll('[data-heading]') || []))
  }, [setHeadings, pathname])
